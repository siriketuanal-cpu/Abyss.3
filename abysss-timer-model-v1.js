...
const idleSnapshot=(timer,at,cap)=>{const start=Number(timer?.start??timer?.idleStart),running=!!(timer?.running??timer?.idleRunning);if(!running||!Number.isFinite(start)||start<=0)return{remaining:null,complete:false};const remaining=Math.max(0,cap-Math.max(0,at-start));return{remaining,complete:remaining<=0,fullAt:start+cap};};
