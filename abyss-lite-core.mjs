...
export const toStoredSlot = slot => { const target = {}; copyStoredSlot(target, slot); return target; };
const snapshotSL = sl => ({
  stamina:{ current:sl.stamina.current, start:sl.stamina.start, running:sl.stamina.running },
  orb:{ current:sl.orb.current, start:sl.orb.start, running:sl.orb.running }
});
...
export function loadStore(storage) {
  ...
  for (let index = 0; index < SLOT_COUNT; index += 1) {
    const slot = normalizeSlot(source[index]);
    slots[index] = slot;
    storedSlots[index] = toStoredSlot(slot);
  }
...
export function saveStore(storage, envelope, slots, storedSlots, changedIndex, sl) {
  ...
  let changed = false;
  if (Number.isInteger(changedIndex) && changedIndex >= 0 && changedIndex < slots.length) {
    const slot = slots[changedIndex];
    if (shouldStore(slot)) { changed = copyStoredSlot(output[changedIndex], slot); slot.dirty = false; }
  } else for (let index = 0; index < slots.length; index += 1) {
    const slot = slots[index];
    if (shouldStore(slot)) { changed = copyStoredSlot(output[index], slot) || changed; slot.dirty = false; }
  }
  if (sl) {
    const previousSL = envelope.sl && typeof envelope.sl === 'object' ? envelope.sl : {};
    const sameSL = ['stamina','orb'].every(key => {
      const before = previousSL[key] || {};
      const after = sl[key];
      return before.current === after.current && before.start === after.start && before.running === after.running;
    });
